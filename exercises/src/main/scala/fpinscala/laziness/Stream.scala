...
trait Stream[+A] {
  ...
  // Implementation should terminate traversal as soon as it encounters a non-matching value.
  //-----------------------------------------------------
  ...
  // NB filter transforms the whole stream but it does so lazily, so find
  ...
  // == BEGIN EXERCISE 5.13 ==================================================================
  // Use unfold to implement map, take, takeWhile, zipWith (see ch 3), and zipAll.
  ...
  //
  //---- map (via unfold) ----------------------------------------
  def map_with_unfold_first_try[B](f: A => B): Stream[B] = unfold(this) {
  ...
  // (now it's correct; it was almost right initially, but I had left off the first case)
  //
  ...
  def zipAll_first_try[B](s2: Stream[B]): Stream[(Option[A], Option[B])] = unfold((this, s2)) {
  ...
  def zipAll_second_try[B](s2: Stream[B]): Stream[(Option[A], Option[B])] = unfold((this, s2)) {
    case (Empty, Empty) => None
    case (Cons(ah, at), Empty) => Some((Some(ah()), Option.empty[B]), (at(), empty[B]))
    case (Empty, Cons(bh, bt)) => Some((Option.empty[A], Some(bh())), (empty[A], bt()))
    case (Cons(ah, at), Cons(bh, bt)) => Some((Some(ah()), Some(bh())), (at(), bt()))
  }

  def zipAll[B](s2: Stream[B]): Stream[(Option[A], Option[B])] = zipWithAll(s2)((_, _))

  //or, making the types explicit, `zipWithAll [B,(Option[A],Option[B])] (s2) ((_,_))`

  def zipWithAll[B, C](s2: Stream[B])(f: (Option[A], Option[B]) => C): Stream[C] =
    Stream.unfold((this, s2)) {
      case (Empty, Empty) => None
      case (Cons(ah, at), Empty) => Some(f(Some(ah()), Option.empty[B]), (at(), empty[B]))
      case (Empty, Cons(bh, bt)) => Some(f(Option.empty[A], Some(bh())), (empty[A], bt()))
  ...
  // First try doesn't work when `this` ends before s.
  //
  ...
  // Second try works, but it's not as pretty as the official solution below.

  def startsWith_third_try[B](s: Stream[B]): Boolean = zipWithAll(s) {
    // Here is the function we will pass to `zipWithAll`.
    def f(oa: Option[A], ob: Option[B]): Boolean = (oa, ob) match {
      case (Some(x), Some(y)) => x == y
      case (_, None) => true
      case _ => false
    }
    f // Pass the function f as the second argument to `zipWithAll`.
  }.foldRight(true)((bool, b) => bool && b)// Finally, take conjunction of all Booleans.
  // Third try works, but still not as pretty as the official solution below.

  // Official solution
	def startsWith[A](s: Stream[A]): Boolean =
		zipAll(s).takeWhile(!_._2.isEmpty) forAll { case (h,h2) => h == h2 }
  // But this should fail if `this` ends before s!
  //
  // == END EXERCISE 5.14 ==================================================================
  ...
  // starting with the original Stream. For example, given `Stream(1,2,3)` it would
  // return `Stream(Stream(1,2,3), Stream(2,3), Stream(3), Stream())`
  def tails_first_try: Stream[Stream[A]] = unfold(this) {
    case Cons(h,t) => Some( (Cons(h,t), t()) )
    case _ => None
  } append Stream(empty)
  // The first try seems to work, but here's the official solution.
...
object Stream {
  ...
  def fibs_first_try: Stream[Int] = {
    ...
    def fibs_aux(n0: Int, n1: Int, acc: Stream[Int]): Stream[Int] =
      fibs_aux(n1, n0+n1, acc append cons(n0, cons(n1, Stream.empty)))
    fibs_aux(0,1,Stream.empty)
  }
  // == END EXERCISE 5.10 ==================================================================

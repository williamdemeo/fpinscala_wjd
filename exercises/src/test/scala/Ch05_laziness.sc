...
object Chapter05_scratch {
  ...
  // takeIf() tests
  Stream(4,2,3,12).takeIf( _ % 2 == 0).toList     //> res15: List[Int] = List(4, 2, 12)
  Stream(1,2,3,4,3,2,1).takeIf( _ < 4).toList     //> res16: List[Int] = List(1, 2, 3, 3, 2, 1)
	// `ones` is equivalent to
	ones.takeIf(_ == 1)                       //> res17: fpinscala.laziness.Stream[Int] = Cons(<function0>,<function0>)
  
  // forAll() tests
  Stream(1,2,3).forAll( _ < 4)                    //> res18: Boolean = true
  Stream(1,2,3).forAll( _ < 3)                    //> res19: Boolean = false
  Stream(1,2,3).forAll( _ > 0)                    //> res20: Boolean = true
  Stream(1,2,3).forAll( _ > 4)                    //> res21: Boolean = false
	ones.forAll(_ != 1)                       //> res22: Boolean = false
	//	ones.forAll(_ == 1)  // results in  java.lang.StackOverflowError
	
  // exists() tests
  Stream(1,2,3).exists( _ < 4)                    //> res23: Boolean = true
  Stream(1,2,3).exists( _ < 3)                    //> res24: Boolean = true
  Stream(1,2,3).exists( _ > 0)                    //> res25: Boolean = true
  Stream(1,2,3).exists( _ > 4)                    //> res26: Boolean = false
	ones.exists(_ != 0)                       //> res27: Boolean = true
